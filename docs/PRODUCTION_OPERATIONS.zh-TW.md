...
每次部署以容器工作流程實際發布的 immutable `sha-<完整 commit SHA>` 作 `VILLAGE_SIEGE_TAG`，例如 `sha-130f5be6ff6f1fe03e72c188b3ab0272343f2ecc`；不要只填裸 SHA。先完成備份與 staging smoke，再更新 production。若新版本 health、version、WSS 或權威 hash 不符，停止新配對並回到前一個 image tag；不要回退 PostgreSQL volume 到未驗證的舊 schema。
