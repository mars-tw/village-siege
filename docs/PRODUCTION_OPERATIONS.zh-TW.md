...
在 `deploy/production.env` 填入實際兩個 hostname、ACME email、完整 Git commit SHA，以及兩個 secret 檔的絕對路徑。先驗證，再啟動：

```sh
docker compose --env-file deploy/production.env -f deploy/compose.production.yaml config --quiet
docker compose --env-file deploy/production.env -f deploy/compose.production.yaml up --build --detach --wait
...
```sh
docker compose --env-file deploy/production.env -f deploy/compose.production.yaml exec -T server \
...
```sh
docker compose --env-file deploy/production.env -f deploy/compose.production.yaml logs --tail 300
docker compose --env-file deploy/production.env -f deploy/compose.production.yaml down
```
