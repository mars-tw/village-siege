...
v2 不再讓一棟兵營生產所有軍隊。每座軍事建築有自己的戰場剪影、成本、施工時間與訓練職責：
...
放置建築時，綠色預覽代表整片占地都可使用；紅色代表至少一格超出地圖、碰到地形、資源或其他建築。完工後，單位會沿可通行格繞過建築，不會直接穿過牆體。攻城棚最寬，建議放在基地外緣並保留出口。
...
手機橫向時使用完全相同的智慧點擊。底部只有一排情境指揮塢：選取工匠後按 **建造**，再用 **下一頁／回首頁** 循環瀏覽三頁建築，選好後點戰場放置；選取各軍事建築則只顯示它能生產的兵種。568×320 下操作列不會和上方資源列重疊；直向時遊戲會暫停並顯示橫放提示。
