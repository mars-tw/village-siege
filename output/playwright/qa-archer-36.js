async page => {
  ...
  await page.waitForFunction(() => {
    ...
    return Boolean(game?.scene?.isActive("CombatShowcaseScene") && scene?.actors?.some(actor => actor.artId === "archer" && actor.team === "player"));
  }, null, { timeout: 20000 });
  ...
  const matrix = await page.evaluate(() => {
    ...
    const game = globalThis.__VILLAGE_SIEGE_DEV_GAME__;
    const scene = game.scene.getScene("CombatShowcaseScene");
    const archer = scene.actors.find(actor => actor.artId === "archer" && actor.team === "player");
    if (!archer) throw new Error("Player archer was not found in CombatShowcaseScene");
    const visual = archer.visual;
    ...
    for (const facing of facings) {
      for (const action of actions) {
        ...
        const expectedTextureKey = `unit-action-sheet-archer-${facing}`;
    ...
    return {
      ...
      actor: { instanceId: archer.instanceId, artId: archer.artId, team: archer.team },
      manifest: {
        id: manifest.id,
  ...
  await saveJson("archer-runtime-facing-action-matrix.json", {
  ...
  await page.screenshot({ path: `${outDir}/archer-runtime-e-attack-frame1.png` });
  representatives.push(await setRepresentativeState("nw", "walk", true));
  await page.waitForTimeout(120);
  await page.screenshot({ path: `${outDir}/archer-runtime-nw-walk-frame1.png` });
  representatives.push(await setRepresentativeState("se", "cast", true));
  await page.waitForTimeout(120);
  await page.screenshot({ path: `${outDir}/archer-runtime-se-cast-frame1.png` });
  ...
  await saveJson("archer-runtime-error-audit.json", errorAudit);
  await saveJson("archer-runtime-representative-states.json", {
    generatedAt: new Date().toISOString(),
    representatives,
  });
