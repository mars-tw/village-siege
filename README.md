...
- 手機點「開始單機戰役」時會在該次使用者操作中要求全螢幕；也可用選村畫面的「全螢幕」、戰場右上角軍令牌或「更多 → 全螢幕」切換。若瀏覽器不支援 Fullscreen API，則改用最大可視滿版。
- 主命令觸控區在 667×375 為約 54×56 CSS px，在 844×390 為約 56×58 CSS px；所有面板均留在遊戲畫布內且不重疊。
