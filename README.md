...
- 戰前選單提供五種 AI 性格：進攻、守備、發展、平衡、襲擾；會改變部署數量、決策節奏與目標偏好。三個村莊也分別提供射程、機動或重裝防禦加成。
...
- 角色動作表改在進入戰場時才載入；戰前選單不再先解碼約 60 MiB 戰鬥圖像。戰鬥使用固定 30 Hz 模擬、畫面更新分流與低頻 HUD 更新。
...
- 底部單列指揮列已改為 Phaser Canvas 遊戲物件，會跟戰場一起縮放；不再使用可能被瀏覽器工具列擠出畫面的 HTML 疊層。
...
- 568×320 的主命令觸控區仍有約 46×48 CSS px；所有面板均留在遊戲畫布內且不重疊。大尺寸觸控平板也會保留完整指揮列。
- 滾輪與雙指縮放、Canvas 內拉近／拉遠、暫停、編隊、框選、移鏡與重新開始都可在固定戰場畫面內完成；每個 Canvas 指令也有鍵盤／螢幕閱讀器代理。
...
- 三村與五種 AI 已改變戰鬥數值與決策偏好，但尚未各自擁有獨立地圖、科技樹、經濟或建築配置，不可宣稱已有三套完整文明玩法。
